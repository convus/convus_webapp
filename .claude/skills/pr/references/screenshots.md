...
- A view at `app/views/u/show.html.erb` → `/u/:username` (pick a representative user from the dev db, e.g. `User.last.username_slug`)
- A component touched by a specific page → screenshot that page
- A shared component (navbar, UI::Badge, etc.) → screenshot 1–2 representative pages that exercise it
...
Invoke the `frontend-screenshots` skill with the `(url-path, page-slug)` pairs from step 1. It handles the dev-server check, signing in (asking the user for credentials when needed — convus_webapp has no seeded users), a PII safety check, viewport sizing, and per-PNG sanity checks. It returns local paths under `tmp/pr_screenshots/<branch>-<page>-<timestamp>-{desktop,mobile}.png`.
...
Rules:
- Each page gets a `### <url-path>` subheading (the literal path, e.g. `/`, `/u/someuser`, `/admin/ratings`) followed by its own table.
