...
- `eval "$(ruby bin/env --export)"` so `$DEV_PORT` and `$BASE_URL` are set. **Each Bash tool call is a fresh shell, so re-export in any shell that reads `$BASE_URL`** — `eval "$(ruby bin/env --export)" && curl …` chained in one call is the safe pattern.
- `curl -fs "$BASE_URL/" >/dev/null` — if it isn't up, **stop and ask the user to start it** (`bin/dev`). `bin/env` resolves `$DEV_PORT` from `$CONDUCTOR_PORT` (defaulting to `3009`), so the `bin/dev` the user starts binds to the same port this skill expects.
