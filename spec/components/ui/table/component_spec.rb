...
RSpec.describe UI::Table::Component, type: :component do
  ...
  # Cache key tests require memory_store (test env uses null_store)
  # TODO: Add caching spec support and re-enable these tests
  context "with cache_key" do
    it "renders with cache_key" do
      result = render_inline(described_class.new(records:, cache_key: "test")) do |table|
        table.column(label: "Name") { |r| r.name }
        table.column(label: "Email") { |r| r.email }
      end

      expect(result).to have_css("td", text: "Alice")
      expect(result).to have_css("td", text: "bob@example.com")
    end
  end
