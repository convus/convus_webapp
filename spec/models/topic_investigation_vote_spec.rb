...
RSpec.describe TopicInvestigationVote, type: :model do
  describe "factory" do
    ...
    it "is valid" do
      ...
      expect(topic_investigation_vote.rating.topics_text).to match(topic_investigation_vote.topic_name)
      # Because we don't automatically run the ReconcileRatingTopicsJob
      expect(topic_investigation_vote.rating.topics.pluck(:id)).to eq([topic_investigation_vote.topic.id])
    end
  end
  ...
  describe "calculate_vote_score" do
    ...
    let(:topic_investigation_vote) { FactoryBot.create(:topic_investigation_vote, topic: topic, rating: rating) }
    ...
    describe "second rating" do
      ...
      let(:rating2) { topic_investigation_vote2.rating }
      ...
      it "is one hire" do
        ...
        expect(topic_investigation_vote.reload.prev_topic_user_ratings.pluck(:id)).to eq([])
        expect(topic_investigation_vote.investigation_user_votes.order(:id).pluck(:id)).to eq vote_ids
        expect(topic_investigation_vote.topic_user_ratings.order(:id).pluck(:id)).to eq rating_ids
        expect(topic_investigation_vote.calculated_vote_score).to eq 1
        expect(topic_investigation_vote2.reload.topic_user_ratings.order(:id).pluck(:id)).to eq rating_ids
        expect(topic_investigation_vote2.id).to be > topic_investigation_vote.id
        expect(topic_investigation_vote2.prev_topic_user_ratings.pluck(:id)).to eq([rating.id])
      ...
      describe "rating is high quality" do
        ...
        it "they are in different tranches" do
          ...
          expect(topic_investigation_vote).to be_valid
          expect(topic_investigation_vote2).to be_valid
          expect(topic_investigation_vote2.rating.default_vote_score).to eq 0
          expect(user.reload.topic_investigation_votes.order(:id).pluck(:id)).to eq vote_ids
          expect(topic.reload.topic_investigation_votes.order(:id).pluck(:id)).to eq vote_ids
          expect(topic_investigation_vote.reload.topic_user_ratings.order(:id).pluck(:id)).to eq rating_ids
